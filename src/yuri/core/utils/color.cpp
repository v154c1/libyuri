...
uint64_t set_64b_from_components(T v0, T v1, T v2, T v3 = ~T())
{
    ...
    return                                           //
        (static_cast<uint64_t>(v0) << (shift * 3)) | //
...
{
    std::array<T, dim>          yuv;
    const std::array<double, 3> rgbf = { { static_cast<double>(rgb[0]) / std::numeric_limits<T>::max(), //
                                           static_cast<double>(rgb[1]) / std::numeric_limits<T>::max(), //
                                           static_cast<double>(rgb[2]) / std::numeric_limits<T>::max() } };

    const auto y = Wr_709 * rgbf[0] + Wg_709 * rgbf[1] + Wb_709 * rgbf[2];
    yuv[0]       = static_cast<T>(std::numeric_limits<T>::max() * (clip_value(y, 0.0, 1.0)));

    yuv[1] = static_cast<T>(std::numeric_limits<T>::max() * (0.5 + clip_value((rgbf[2] - y) * Kb_709, -0.5, 0.5)));

    yuv[2] = static_cast<T>(std::numeric_limits<T>::max() * (0.5 + clip_value((rgbf[0] - y) * Kr_709, -0.5, 0.5)));
    if (dim > 3) {
        yuv[3] = rgb[3];
    }
    return yuv;
...
{
    std::array<T, dim>          rgb;
    const std::array<double, 3> yuvf = { { static_cast<double>(yuv[0]) / std::numeric_limits<T>::max(),       //
                                           static_cast<double>(yuv[1]) / std::numeric_limits<T>::max() - 0.5, //
                                           static_cast<double>(yuv[2]) / std::numeric_limits<T>::max() - 0.5 } };
    rgb[0] = static_cast<T>(std::numeric_limits<T>::max() * clip_value(yuvf[0] + (yuvf[2] / Kr_709), 0.0, 1.0));
    rgb[1] = static_cast<T>(std::numeric_limits<T>::max() * clip_value(yuvf[0] - yuvf[2] * WrKrWg_709 - yuvf[1] * WbKbWg_709, 0.0, 1.0));
    rgb[2] = static_cast<T>(std::numeric_limits<T>::max() * clip_value(yuvf[0] + yuvf[1] / Kb_709, 0.0, 1.0));
    if (dim > 3) {
        rgb[3] = yuv[3];
    }
    return rgb;
...
std::istream& operator>>(std::istream& is, color_t& col_out)
{
    color_t col;
    char    c;
    enum class state_t { start, data };
    enum class csp_t { rgb, rgb16, yuv, yuv16 };
