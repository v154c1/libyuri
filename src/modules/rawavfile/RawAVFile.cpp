...
namespace yuri {
    namespace rawavfile {
        ...
        core::Parameters RawAVFile::configure() {
            ...
            p.set_description(
                    "rawavsource reads video files using ffmpeg. It can decode the video or output the encoded frames.");
        ...
        RawAVFile::RawAVFile(const log::Log &_log, core::pwThreadBase parent, const core::Parameters &parameters)
            ...
            if (filename_.empty()) {
                ...
                if (!open_file(filename_)) {
                    ...
                    log[log::warning]
                            << "Failed to open file, but allow_empty was specified, so waiting for new filename";
                }
            }
        }
        ...
        bool RawAVFile::open_file(const std::string &filename) {
            ...
            for (size_t i = 0; i < video_streams_.size(); ++i) {
                ...
                if (!decode_ && video_streams_[i].format != core::compressed_frame::h264) {
                    ...
                    log[log::info] << "Found video stream with format "
                                   << get_format_name_no_throw(video_streams_[i].format) << " and resolution "
                                   << video_streams_[i].resolution << ". Decoding to "
                                   << get_format_name_no_throw(video_streams_[i].format_out);
                }
            }
//    if (decode_) {
            for (auto &stream: audio_streams_) {
                ...
                log[log::info] << "Found audio stream, format:" << get_format_name_no_throw(stream.format)
                               << ", decoding to "
                ...
                if (frames_[i]) {
                    if (fps_ >= 0 && !ignore_timestamps_) {
                        ...
                        if (curr_time < next_times_[i]) {
                            ...
                            log[log::debug] << "Pushing frame for " << i << " delayed by "
                                            << (timestamp_t{} - next_times_[i]) << "ms";
...
//            stream.
        ...
        bool RawAVFile::process_undecoded_frame(index_t idx, const AVPacket &packet) {
            ...
            if (format == core::compressed_frame::h264) {
                if (libav::get_opt<bool>(video_streams_[idx].ctx->priv_data, "is_avc")) {
                    ...
                    if (nal_length_size > 4 || nal_length_size < 0) {
                        log[log::warning] << "Received invalid nal_length_size: " << nal_length_size
                                          << ", ignoring frame";
                        return false;
                    }
            ...
            if (format == core::compressed_frame::h264 || format == core::compressed_frame::avc1) {
                ...
                if (frame_usable_for_extradata && ((last_params_emitted_ < 0) || (emit_params_interval_ > 0 &&
                                                                                  (++last_params_emitted_ ==
                                                                                   emit_params_interval_)))) {
            ...
            if ((extra_bytes + sps_pps_len) == 0) {
                f = core::CompressedVideoFrame::create_empty(format, video_streams_[idx].resolution, packet.data,
                                                             packet.size);
            } else {
                f = core::CompressedVideoFrame::create_empty(format, video_streams_[idx].resolution,
                                                             packet.size + extra_bytes + sps_pps_len);
                if (sps_pps_len > 0) {
            ...
            duration_t dur = 1_s * packet.duration * video_streams_[idx].stream->avg_frame_rate.den /
                             video_streams_[idx].stream->avg_frame_rate.num;
            if (!dur.value)
                dur = video_streams_[idx].delta;
            f->set_duration(dur);

            log[log::debug] << "Found packet!" /* (pts: " << pts << ", dts: " << dts <<*/ ", dur: " << dur;
            log[log::debug] << "num/den:" << video_streams_[idx].stream->avg_frame_rate.num << "/"
                            << video_streams_[idx].stream->avg_frame_rate.den;
...
*/
        ...
        bool RawAVFile::decode_audio_frame(index_t idx, const AVPacket &packet, AVFrame *av_frame, bool &keep_packet) {
            ...
            if (!audio_streams_[idx].swr_ctx) {
                ...
                auto output_sample_count = av_rescale_rnd(av_frame->nb_samples, audio_streams_[idx].sample_rate,
                                                          audio_streams_[idx].ctx->sample_rate, AV_ROUND_UP);
                auto f = core::RawAudioFrame::create_empty(audio_streams_[idx].format_out, av_frame->channels,
                                                           audio_streams_[idx].sample_rate,
                                                           output_sample_count);
                auto out_buffer = f->data();
                const auto **in_buffer = const_cast<const uint8_t **>(av_frame->data);
                auto ret2 = swr_convert(audio_streams_[idx].swr_ctx.get(), &out_buffer, av_frame->nb_samples, in_buffer,
                                        av_frame->nb_samples);
                auto real_buffer_size
                        = av_samples_get_buffer_size(nullptr, av_frame->channels, ret2,
                                                     libav::avsampleformat_from_yuri(audio_streams_[idx].format_out),
                                                     1);
                log[log::verbose_debug] << "Received " << av_frame->nb_samples << " samples, expected to convert to "
                                        << output_sample_count << ", actually got " << ret2
...
//    av_init_packet(&empty_packet);
            ...
            while (still_running()) {
                ...
                if (idx >= 0) {
                    if (frames_[idx]) {
                        log[log::warning] << "Overwriting frame in for index " << idx;
                    }
                    if (!decode_) {
